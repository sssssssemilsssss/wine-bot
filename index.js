...
function buildPositionsRow(list) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`wine_claimposition_${list.id}`)
...
// Освобождает позицию, которую занимал пользователь (например, при выходе из набора).
...
async function handleButton(interaction) {
  ...
  if (action === 'leave') {
    ...
    const posChanged = freeUserPosition(list, interaction.user.id);
    saveLists(lists);
    await interaction.update({ embeds: [buildEmbed(list)], components: [buildMainRow(list)] });
  ...
  if (action === 'manage') {
    ...
    const row2Buttons = [
      new ButtonBuilder()
        ...
        .setLabel(list.threadId ? 'Ветка уже создана' : 'Создать ветку')
        .setEmoji('🧵')
        .setStyle(ButtonStyle.Secondary)
  ...
  if (action === 'claimposition') {
    ...
    const modal = new ModalBuilder()
      .setCustomId(`wine_claimposmodal_${listId}`)
...
async function handleUserSelect(interaction) {
  ...
  if (action === 'selectremoveperson') {
    ...
    const posChanged = freeUserPosition(list, targetId);
    saveLists(lists);
    await updateListMessage(guild, list);
...
async function handleModalSubmit(interaction) {
  ...
  if (action === 'threadmodal') {
    ...
    try {
      ...
      // Ветка создаётся от самого сообщения со сбором, чтобы быть прямо под ним.
  ...
  if (action === 'claimposmodal') {
    ...
    // Освобождаем предыдущую позицию этого же пользователя, если была занята.
