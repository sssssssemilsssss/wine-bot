...
// Отправляет сообщение об ошибке независимо от того, был ли interaction уже
// отвечен/отложен — раньше такие ошибки просто пропадали молча.
...
function buildEmbed(list) {
  const emoji = MODE_EMOJI[list.mode] || '📋';
  const qtyText =
  ...
  if (list.admins.length) {
    fields.push({
      ...
      value: list.admins.map((id) => `<@${id}>`).join(', '),
      inline: true,
    });
  }
  ...
  const lines = list.participants.length
    ...
    : ['_пока никого — нажми «Присоединиться» ✅_'];
  ...
  const embed = new EmbedBuilder()
    .setTitle(`${emoji} Набор: ${list.mode}`)
    .setColor(isFull(list) ? 0x2ecc71 : 0x5865f2)
    .addFields(fields)
    .setTimestamp();

  if (list.threadId) {
    embed.setDescription(`🧵 Создана ветка: <#${list.threadId}>`);
  }

  embed.setFooter({
    text: isFull(list) ? 'Набор заполнен 🔒' : `ID набора: ${list.id}`,
  });
...
async function handleWineCommand(interaction) {
  ...
  // Тегаем участников роли отдельным блоком: если тут что-то пойдёт не так,
  // это не должно ломать уже отправленный эмбед — просто сообщим об ошибке.
  try {
...
async function handleButton(interaction) {
  ...
  if (action === 'manage') {
    ...
    await interaction.reply({
      content: '⚙️ Панель управления набором:',
...
async function handleModalSubmit(interaction) {
  ...
  // Всё, что может пойти не так при создании ветки, теперь ловится тут и
  // показывается пользователю текстом ошибки вместо тихого "ничего не произошло".
  try {
    const guild = interaction.guild;
    const channel = await guild.channels.fetch(list.channelId);

    if (!channel || !channel.threads) {
      throw new Error('этот канал не поддерживает ветки');
    }
