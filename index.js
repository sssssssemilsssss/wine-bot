...
const FINAL_PING_USER_IDS = (
  ...
  .map((s) => s.trim())
  .filter(Boolean);
// Ветки отчётов всегда создаются в этом канале, независимо от того, где написали "!ветка".
const BRANCH_TARGET_CHANNEL_ID = process.env.BRANCH_TARGET_CHANNEL_ID || '1536276232877121536';
// Роль, которая всегда видит ветку, даже когда доступ для остальных скрыт кнопкой.
const BRANCH_TOP_ROLE_ID = process.env.BRANCH_TOP_ROLE_ID || '1515042126948335725';

// ---------- Приватные голосовые комнаты ----------
// Триггер-канал и категория для системы приватных войсов — всегда фиксированные,
// не зависят от /voice-setup и не настраиваются через команду.
const VOICE_TRIGGER_CHANNEL_ID = process.env.VOICE_TRIGGER_CHANNEL_ID || '1528042843149697084';
const VOICE_CATEGORY_ID = process.env.VOICE_CATEGORY_ID || '1528042302445060106';
...
const EMOJI = {
  ...
  assignPosition: '🆕',
  removePosition: '🗑️',
  branchAddParticipant: '➕',
  branchRemoveParticipant: '➖',
  branchHide: '🙈',
  branchShow: '👁️',
};
...
// (например нет persistent volume). Эти строки в логе сразу показывают, откуда бот
// читает данные и сколько реально загрузил — если после деплоя тут 0 при том что до
// деплоя что-то было создано, значит файлы на диске не пережили деплой.
console.log(
...
process.on('SIGINT', () => {
  console.log('[shutdown] получен SIGINT, завершаюсь штатно.');
  process.exit(0);
});
...
// Кнопки под статусным эмбедом ветки — доступны всё время (не только когда все залили),
// чтобы составом и закрытием/удалением ветки можно было управлять в один клик.
function buildBranchControlsRow(branch) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(cid('branch', 'addparticipant', branch.id))
      .setLabel('Добавить участника')
      .setEmoji(EMOJI.branchAddParticipant)
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId(cid('branch', 'removeparticipant', branch.id))
      .setLabel('Удалить участника')
      .setEmoji(EMOJI.branchRemoveParticipant)
      .setStyle(ButtonStyle.Secondary),
    new ButtonBuilder()
      .setCustomId(cid('branch', 'togglevisibility', branch.id))
      .setLabel(branch.hidden ? 'Показать доступ' : 'Скрыть доступ')
      .setEmoji(branch.hidden ? EMOJI.branchShow : EMOJI.branchHide)
      .setStyle(branch.hidden ? ButtonStyle.Success : ButtonStyle.Secondary),
    new ButtonBuilder()
...
async function handleBranchAllSubmitted(thread, branch) {
  ...
  try {
    ...
    await thread.send({ embeds: [listEmbed] });
  } catch (e) {
...
async function handleBranchButton(interaction) {
  ...
  if (action === 'addparticipant' || action === 'removeparticipant') {
    ...
    const select = new UserSelectMenuBuilder()
      .setCustomId(cid('branch', action === 'addparticipant' ? 'selectadd' : 'selectremove', branchId))
      .setPlaceholder(action === 'addparticipant' ? 'Кого добавить в ветку' : 'Кого убрать из ветки')
      .setMinValues(1)
      .setMaxValues(25);
  ...
  if (action === 'togglevisibility') {
    ...
    try {
      ...
      if (!branch.hidden) {
        ...
        await Promise.all(
          [...currentMembers.values()]
            ...
            .map((m) => thread.members.remove(m.id).catch(() => {}))
        );
        branch.hidden = true;
      } else {
      ...
      saveBranches(branches);
      await updateBranchStatusMessage(interaction.guild, branch);
      await interaction.editReply({ content: branch.hidden ? '🙈 Доступ скрыт — видят только ответственные.' : '👁️ Доступ возвращён участникам.' });
    } catch (e) {
      console.error('Ошибка переключения видимости ветки отчётов:', e);
      await interaction.editReply({ content: `⚠️ Не удалось изменить доступ: ${e.message || e}` });
    }
    return;
  }
...
// Обработка выбора пользователей из меню, открытого кнопками "Добавить/удалить участника".
async function handleBranchUserSelect(interaction) {
  const [, action, branchId] = parseCid(interaction.customId);
  const branch = branches.get(branchId);
  if (!branch) {
  ...
  if (action === 'selectadd') {
    ...
    for (const id of targetIds) {
      if (!branch.participants.some((p) => p.id === id)) {
        branch.participants.push({ id, submitted: false, link: null, reportText: null, submittedAt: null });
        added.push(id);
      }
    }
    ...
    await interaction.update({
      content: added.length ? `➕ Добавлены: ${added.map((id) => `<@${id}>`).join(', ')}` : 'ℹ️ Уже были в составе.',
      components: [],
    });
    return;
  }
  ...
  if (action === 'selectremove') {
    const removeIds = new Set(targetIds);
    ...
    if (removed && branch.participants.length > 0 && getUnsubmittedParticipants(branch).length === 0) {
      const thread = await guild.channels.fetch(branch.threadId).catch(() => null);
      if (thread) await handleBranchAllSubmitted(thread, branch);
    }
    return;
  }
}
...
async function tryCreateBranch(message) {
  ...
  let targetChannel;
  try {
    targetChannel = await message.guild.channels.fetch(BRANCH_TARGET_CHANNEL_ID);
  } catch (e) {
    console.error('Не удалось получить канал для веток отчётов:', e);
  }
  if (!targetChannel) {
    await message.reply({ content: '⚠️ Не найден канал для веток отчётов — сообщи об этом администратору.' });
    return true;
  }
  ...
  try {
    ...
    await message.reply({
      content:
        ...
        'Проверь, что у бота есть права «Создавать приватные ветки» и «Управлять ветками» в целевом канале.',
    });
    return true;
  }
  ...
  let statusMsg;
  try {
    statusMsg = await thread.send({ embeds: [buildBranchEmbed(branch)], components: [buildBranchControlsRow(branch)] });
  ...
  // Подтверждение видит по факту только автор — сообщение адресовано лично ему
  // и само удаляется через несколько секунд, чтобы не оставалось в канале для остальных.
  const confirm = await message.channel
    .send({ content: `<@${message.author.id}> ✅ Ветка «${name}» создана: ${thread.toString()}` })
    .catch(() => null);
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  try {
    ...
    // Зашёл в триггер-канал — создаём комнату. Триггер и категория фиксированные
    // (VOICE_TRIGGER_CHANNEL_ID / VOICE_CATEGORY_ID), не зависят от настроек сервера.
    if (
      newState.channelId === VOICE_TRIGGER_CHANNEL_ID &&
      newState.channelId !== oldState.channelId
    ) {
      await createVoiceRoom(guild, newState.member, { categoryId: VOICE_CATEGORY_ID });
    }
  ...
  try {
    if (interaction.isChatInputCommand() && interaction.commandName === 'wine') {
      ...
      if (prefix === 'voice') {
        ...
        await handleBranchUserSelect(interaction);
      } else {
